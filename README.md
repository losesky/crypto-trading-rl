...
\
## Reward Function for Capital Preservation (Updated May 14, 2025)

The agent's reward function is designed to strongly discourage losing money and to protect capital. The reward at each step is:

- Realized losses are penalized more heavily (multiplied by 1.5).
- If a position is open, discounted unrealized PnL is added.
- If capital drops below the initial amount, a fixed penalty is subtracted from the reward.

**Mathematical formula:**

    reward =
        (realized_pnl * LOSS_AVERSION_MULTIPLIER if realized_pnl < 0 else realized_pnl)
        + (unrealized_pnl * lambda^holding_period if position is open)
        - (CAPITAL_PRESERVATION_PENALTY_AMOUNT if capital < initial_capital)

This encourages the agent to avoid losses and maintain or grow its capital over time.
