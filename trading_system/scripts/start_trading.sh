...
show_help() {
    ...
    echo "  --menu                显示交互elif [ "$UI_ONLY" = true ]; then
    # 只启动UI界面、WebSocket代理和API代理
...
" > "$TRADING_SYSTEM_DIR/logs/websocket_proxy.log" 2>&1 &
    ...
    if ps -p $WS_PROXY_PID > /dev/null; then
        echo -e "${GREEN}WebSocket代理已成功启动${NC}"
        # 检查日志中是否有错误
...
start_api_proxy() {
    ...
    if [ -f "$API_PROXY_LAUNCHER" ]; then
        ...
        cd "$TRADING_SYSTEM_DIR/src"
        python "$API_PROXY_LAUNCHER" --ws-url "$WS_URL" --trading-url "$TRADING_URL" > "$TRADING_SYSTEM_DIR/logs/api_proxy.log" 2>&1 &
    else
        # 直接内联启动
